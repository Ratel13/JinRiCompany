...
#define classPlistName_macro "S15ShowVideo"
#define zNum 100
#define btnTag 100

CCScene* S15ShowVideo::scene()
{
	CCScene * scene = NULL;
	do
	{
		scene = CCScene::create();
		S15ShowVideo *layer = S15ShowVideo::create();
...
void S15ShowVideo::refreshDownLoadingData(bool isCompleted)
{
    if (isCompleted)
    {
...
void S15ShowVideo::DownLoadFinish()
{
...
bool S15ShowVideo::setUpSubClass2()
{
	bool bRet = false;
	do
	{
        ...
        
        {
            ...
            CCMenuItemImage * aItemImage = CCMenuItemImage::create(str.c_str(),
                                                                   str.c_str(),
                                                                   this,
                                                                   menu_selector(S15ShowVideo::S15DetailCallback));
...
//                    CCLOG("aaaaa%s",str.c_str());
            ...
            if(i==0){
                const char * imageName = "S15ScrollBigPic1.png";//showBigPicMap[PersonalApi::convertIntToString(i+1)].c_str();
                //CCLog("%s",imageName);
                // S15ScrollBigPic1.png
...
void S15ShowVideo::S15DetailCallback(CCObject* pSender)
{
    if (isMenuLock == false)
    {
...
void S15ShowVideo::scrollViewDidScroll(CCScrollView* view)
{
    isMenuLock = true;
    this->schedule(schedule_selector(S15ShowVideo::S31Delay),0.5f);
}

void S15ShowVideo::S31Delay(float dt)
{
    isMenuLock = false;
}
