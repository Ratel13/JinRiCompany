...
#define classPlistName_macro "S55ShowVideo"
#define zNum 100
#define btnTag 100

CCScene* S55ShowVideo::scene()
{
	CCScene * scene = NULL;
	do
	{
		scene = CCScene::create();
		S55ShowVideo *layer = S55ShowVideo::create();
...
void S55ShowVideo::refreshDownLoadingData(bool isCompleted)
{
    if (isCompleted)
    {
...
void S55ShowVideo::DownLoadFinish()
{
	...
	if (root_smallpicture && root_smallpicture->type==Json_Array)
	{
        ...
        for(map<string, string>::iterator iter=showPicMap.begin();iter!=showPicMap.end();++iter)
        {
            CCLOG("11111==%s",iter->second.c_str());
            // π”√ * ∑√Œ µ¸¥˙∆˜À˘÷∏œÚµƒ‘™Àÿ
        }
	}
...
bool S55ShowVideo::setUpSubClass2()
{
	bool bRet = false;
	do
	{
        ...
        
        {
            ...
            CCMenuItemImage * aItemImage = CCMenuItemImage::create(str.c_str(),
                                                                   str.c_str(),
                                                                   this,
                                                                   menu_selector(S55ShowVideo::S15DetailCallback));
...
//                    CCLOG("aaaaa%s",str.c_str());
            ...
            if(i==0){
                const char * imageName = "S65ScrollBigPic1.png";//showBigPicMap[PersonalApi::convertIntToString(i+1)].c_str();
...
void S55ShowVideo::S15DetailCallback(CCObject* pSender)
{
    if (isMenuLock == false)
    {
        showStrUrlMap = PersonalApi::getRealGroupStringMap(plistDic,"S65VideoIntro","videoIntro");
...
void S55ShowVideo::scrollViewDidScroll(CCScrollView* view)
{
    isMenuLock = true;
    this->schedule(schedule_selector(S55ShowVideo::S31Delay),0.5f);
}

void S55ShowVideo::S31Delay(float dt)
{
    isMenuLock = false;
}
