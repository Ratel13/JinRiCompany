...
#define classPlistName_macro "S45ShowVideo"
#define zNum 100
#define btnTag 100

CCScene* S45ShowVideo::scene()
{
	CCScene * scene = NULL;
	do
	{
		scene = CCScene::create();
		S45ShowVideo *layer = S45ShowVideo::create();
...
void S45ShowVideo::refreshDownLoadingData(bool isCompleted)
{
...
void S45ShowVideo::DownLoadFinish()
{
		...
		while(json_child)
		{
            ...
            if (i==0) {
                ...
                if(child_pic_url){
                    char szsamllpicname[512]={0};
                    sprintf(szsamllpicname, "S%d5ScrollPic%d.png",AppDelegate::S1NaviSelected,k+1);
                    CCLog("%d",AppDelegate::S1NaviSelected);
                    string picNameStr(szsamllpicname);
                    showPicMap[PersonalApi::convertIntToString(k+1)] = picNameStr;
                    DownLoadFile(child_pic_url->valuestring,szsamllpicname);
...
bool S45ShowVideo::setUpSubClass2()
{
	bool bRet = false;
	do
	{
        ...
        
        {
            ...
            CCMenuItemImage * aItemImage = CCMenuItemImage::create(str.c_str(),
                                                                   str.c_str(),
                                                                   this,
                                                                   menu_selector(S45ShowVideo::S15DetailCallback));
...
//                    CCLOG("aaaaa%s",str.c_str());
            ...
            if(i==0)
            {
                const char * imageName = "S55ScrollBigPic1.png";//showBigPicMap[PersonalApi::convertIntToString(i+1)].c_str();
...
void S45ShowVideo::S15DetailCallback(CCObject* pSender)
{
    if (isMenuLock == false)
    {
        showStrUrlMap = PersonalApi::getRealGroupStringMap(plistDic,"S55VideoIntro","videoIntro");
...
void S45ShowVideo::scrollViewDidScroll(CCScrollView* view)
{
    isMenuLock = true;
    this->schedule(schedule_selector(S45ShowVideo::S31Delay),0.5f);
}

void S45ShowVideo::S31Delay(float dt)
{
    isMenuLock = false;
}
